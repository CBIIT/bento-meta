...
def main(
    ...
    base_props_terms = tmdb.get_props_and_terms_by_model(base_model)
    comp_props_terms = tmdb.get_props_and_terms_by_model(comp_model)
    ...
    for pt_dict in base_props_terms:
        ...
        prop_synonyms = tmdb.get_property_synonyms(prop)
        ...
        if not prop_synonyms:
            ...
            row['num_shared_terms'] = f"{str(0)}/{str(len(base_term_val_set))} (both/{base_model})"
            row_list.append(row)
            continue

        # check if synonyms in comp_props_terms
        syn_nanos = []
        for result in prop_synonyms:
            for syn in result:
                syn_nano = result[syn]['nanoid']
                # add syn_nano if it is in comp model's nano list and not duplicated
                if syn_nano in comp_nanos and syn_nano not in syn_nanos:
                    syn_nanos.append(syn_nano)
        ...
        for nano in syn_nanos:
            ...
            row['num_shared_terms'] = f"{str(len(terms_both))}/{str(len(base_term_val_set))} (both/{base_model})"
