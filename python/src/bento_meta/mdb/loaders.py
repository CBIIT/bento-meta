...
from bento_meta.util.cypher.entities import N, R, _plain_var
...
def _cEntity(ent, model, _commit):
    ...
    # special handling
    if label == "term":
        cEnt = N(label=label, props={"value": ent.value})
        if ent.origin_name:
        ...
        cEnt = N(label="value_set", props={"handle": ent.handle})
        if ent.url:
            cEnt._add_props({"url": ent.url})
    elif label == "concept":
        cEnt = N(label="concept")
    elif label == "tag":
        cEnt = N(label="tag", props={"key": ent.key, "value": ent.value})

    else:
        cEnt = N(label=label, props={"handle": ent.handle, "model": model.handle})
    # all ents
    if _commit:
    ...
    for tm in ent.concept.terms.values():
        ...
        stmts.extend(
            [
                ...
                Statement(
                    Match(cEnt),
                    Merge(R(Type="has_concept").relate(_plain_var(cEnt), cConcept)),
                    use_params=True,
                ),
                Statement(
                    Match(R(Type="has_concept").relate(cEnt, cConcept), cTerm),
                    Merge(
