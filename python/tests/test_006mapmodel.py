...
def test_put_model(bento_neo4j):
    ...
    with m.drv.session() as session:
        result = session.run(
            'match (v:value_set)-->(t:term {value:"electric_boogaloo"}) return v,t',
        )
        rec = result.single()
    ...
    with m.drv.session() as session:
        ...
        result = session.run(
            "match (c:concept) where id(c)=$id return c",
            {"id": concept.neoid},
        )
    ...
    with m.drv.session() as session:
        result = session.run(
            'match (n:node)<-[:has_src]-(r:relationship {handle:"at_enrollment"})-[:has_dst]->(:node {handle:"enrollment"}) where id(n)=$id return r',
            {"id": prior_surgery.neoid},
        )
        s = result.single()
        assert s
    ...
    assert at_enrollment not in m.edges_out(prior_surgery)
    m.dput()
    with m.drv.session() as session:
        result = session.run(
            'match (n:node)<-[:has_src]-(r:relationship {handle:"at_enrollment"})-[:has_dst]->(:node {handle:"enrollment"}) where id(n)=$id return r',
