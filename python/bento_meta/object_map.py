            ...
            for att in self.cls.mapspec()["relationship"]:
                ...
                for rec in result:
                    ...
                    if o:
                        ...
                        c = None
                        for l_ in rec["a"].labels:
                            c = ObjectMap.cls_by_label(l_)
                            if c:
                                break
                        if not c:
                            raise RuntimeError(
    ...
    def get_attr_q(self, obj, att):
        ...
        if att in self.cls.mapspec()["property"]:
            ...
            if len(end_lbls) == 1:
                ...
                cond = []
                for l_ in end_lbls:
                    cond.append("'{lbl}' IN labels(a)".format(lbl=l_))
                cond = " OR ".join(cond)
    ...
    def put_attr_q(self, obj, att, values):
        ...
        if att in self.cls.mapspec()["property"]:
            ...
            if not self._check_values_list(att, values):
                raise ArgError(
                    "'values' must be a list of mapped Entity objects of the appropriate subclass for attribute '{att}'".format(
                        att=att
                    )
                )
            stmts = []
