...
While the above details the schema, the [point](#point) of PFB is to encode _data_ with it, so that it can be stored and sent. A complete worked example using the ICDC data model, with further explanatory text and runnable code, is available in [this Jupyter notebook](./icdc-pfb-example.ipynb). 
