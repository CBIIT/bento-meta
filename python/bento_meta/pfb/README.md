...
the node data being sent should be connected by "edges", "links", or "relationships" to other nodes of different types. See below.
...
The ``Relation`` schema (apparently) exists to contain an indiciation of the precise _instances_ of graph nodes to which the accompanying data node(s) link.  ``Link`` generically defines the local graph structure, while ``Relation`` imparts the instance information. There are probably other ways to do it.
