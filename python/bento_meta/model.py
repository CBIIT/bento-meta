...
class Model(object):
    def __init__(self, handle=None, drv=None):
        ...
        :param str handle: A string name for the model; is the model property in MDB database nodes.
        :param neo4j.Driver drv: A `neo4j.Driver` object for the database connection
        (see :class:`neo4j.GraphDatabase`)
        """
        if not handle:
