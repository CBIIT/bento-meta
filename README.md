...
- synonyms for local vocabulary mapped from external standards; 
- the value sets for properties with enumerated value domains, and data types for other properties; and
- property-to-property data transformations.

A production instance of and MDB contains the "official" representation of a data model, in that it records the curated external terminology mappings and official sets of valid terms for each relevant property. In this way, the MDB is an extension of the [MDF](https://github.com/CBIIT/bento-mdf) for any model it contains.
...
APIs for working with the MDB are available in [Python](./python).
...
A Transform node marks two sets of Properties to indicate that values from one set (the inputs) can be transformed into valid values of the other set (the outputs). The operations that perform the transformation are recorded in Transform Step nodes.

### Transform Step

Transform Step nodes contain Python package names and code endpoints (module function names) that accept generic positional or keyword arguments and produce scalar, list, or dict return values. A Transform Step may also contain a `params` property, a JSON representation of a dict with literal values used as parameters for the function.

Transform nodes point to a linked series of Transform Steps that when executed in order on input property values yield output property values. 
